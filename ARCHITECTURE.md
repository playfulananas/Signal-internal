...
| 27 | 2026-09-04 | **Turn-readiness UI + manual-test hotfixes.** Added per-Unit attack/Direct-HQ badges and an authoritative End Turn forecast; fixed command-destroyed Units remaining on their tiles; serialized the host's initial online-state write before mulligan and added a pre-play snapshot schema guard. Integrated the combined build into internal `main`; the client-testing repository remains untouched. |
...
| `js/sync.js` | `stateRevision`, `prepareVersionedState`, `shouldAcceptRemoteState`, `isPrePlayMulliganSnapshot`, `normalizeRemoteUnit`, `normalizeRemoteBoard` — pure online revision, setup-schema, and compatibility helpers | nothing |
